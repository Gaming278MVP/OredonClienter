...
if (!args[0]) {
  ...
  msg.channel.send(embed); // Test dulu yah!
     ...
     client.memory.wal(`ModLog.${msg.guild.id}.on`, true)
     msg.channel.send(`<:toggleon:534669824811466762> Modlog has been enabled!`);
  } else if(args[0] == 'off') {
     client.memory.wal(`ModLog.${msg.guild.id}.on`, false)
    ...
    if(!channel) return msg.channel.send("Please specify a mention channels!")
     ...
     client.memory.wal(`ModLog.${msg.guild.id}.channel`, channel.id).then(ModLog => {
